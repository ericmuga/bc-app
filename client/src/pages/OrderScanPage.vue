<template>
  <div class="page">
    <div class="page-header">
      <h2 class="page-title">Order Scan</h2>
      <p class="text-muted text-sm">Scan barcode or search by order number</p>
    </div>
    ...
    <div v-else-if="doc" class="mt-4">
      ...
      <div class="bc-card" :class="{ 'card-warning': isCopy }">
        ...
        <div class="doc-header">
          <div>
            <div class="doc-no mono">{{ doc.header.OrderNo }}</div>
          ...
          <div class="doc-meta">
            ...
            <div class="meta-grid">
              ...
              <span class="text-sm">{{ fmtDay(doc.header.OrderDate) }}</span>
...
async function lookup() {
  ...
  reset()
  try {
    const { data } = await ordersApi.get(no)
    doc.value = data
    await loadAudit(no)
  } catch (e) {
...
async function doConfirm() {
  const result = await confirm(doc.value.header.OrderNo)
...
function toggleAudit() {
  ...
  if (showAudit.value && doc.value) loadAudit(doc.value.header.OrderNo)
}
...
.doc-customer  { font-size: 16px; margin-top: 2px; }
